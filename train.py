    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt in trainloader:
        ...
        try:
            ...
            if config.max_split == 0:
                ...
                #     if epoch > 8:
                #         e = epoch - 8
                #         loss_adv, outputs = model(src, lengths, dec, targets, teacher_ratio=0.9 ** e)
                #     else:
                #         loss_adv, outputs = model(src, lengths, dec, targets)
                # else:
                #     loss_adv, outputs = model(src, lengths, dec, targets)
                # loss_adv.backward()
                # fgm.restore()
...
def main():
    ...
    if opt.mode == "train":
        ...
        score = eval_model(model, data, params, mode='test')
