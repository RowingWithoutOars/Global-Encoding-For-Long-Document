...
from RandWireNN.RandomlyWired import CNN
...
class rnn_encoder(nn.Module):
    ...
    def __init__(self, config, embedding=None):
        ...
        if config.rwnn:
            self.rwnn = CNN(config, num_classes=config.emb_size, nxpath=config.nxpath)

        if config.selfatt: # 自注意力
        ...
        elif config.cell == 'moglstm':
            self.rnn = MogLSTM(config.emb_size, config.hidden_size, mog_iterations=2)
        else:
    ...
    def __init__(self, config, embedding=None, use_attention=True):
        ...
        if config.cell == 'gru':
            ...
            self.rnn = StackedLSTM(input_size=input_size, hidden_size=config.hidden_size,
                                   num_layers=config.dec_num_layers, dropout=config.dropout)
        # elif config.cell == 'moglstm':
        #     self.rnn = StackedMogLSTM(input_size=input_size, hidden_size=config.hidden_size,
        #                               num_layers=config.dec_num_layers, dropout=config.dropout)
        else:
            self.rnn = StackedLSTM(input_size=input_size, hidden_size=config.hidden_size,
                                   num_layers=config.dec_num_layers, dropout=config.dropout)
...
class StackedLSTM(nn.Module):
    ...
    def forward(self, input, hidden):
        h_0, c_0 = hidden  # (128,512)
        h_1, c_1 = [], []
        for i, layer in enumerate(self.layers):
            h_1_i, c_1_i = layer(input, (h_0[i], c_0[i]))  # (128,512), (128,512)
            input = h_1_i
...
class StackedMogLSTM(nn.Module):
    def __init__(self, num_layers, input_size, hidden_size, dropout):
        super(StackedMogLSTM, self).__init__()
        self.dropout = nn.Dropout(dropout)
        self.num_layers = num_layers
        self.layers = nn.ModuleList()

        for i in range(num_layers):
            self.layers.append(MogLSTM(input_size, hidden_size, mog_iterations=2))
            input_size = hidden_size
